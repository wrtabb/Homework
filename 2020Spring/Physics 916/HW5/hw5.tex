...
\begin{homeworkProblem}
	...
	\[
		...
		\end{split} 
	\]
	So we know the value of the diagonal of $K$ and thus $\tilde K$ and we also know that they are Hermitian. $\tilde K = 1/2[H_{11}-H_{22}]K$ and $H_{11}-H_{22}$ is real.
	\\
	\\
...
\begin{homeworkProblem} 
	...
	\\
	\\
	2. What is the probability that a measurement of spin along the z-axis will find $-\hbar/2$? What is the state of the system immediately after measurement?
	\\
	\\
	3. What is the probability that a measurement of spin along the z-axis will find $+\hbar/2$? What is the state of the system immediately after measurement?
	\\
	\\
	4. What is the probability that spin down along the y-axis? What is the state of the system immediately after measurement?
	\\
	\\
	5. What is the probability for spin up along the x-axis? What is the state of the
	system immediately after the measurement?
	\\
	\\
	6. What is the probability that a measurement of spin along the u-axis will find $-\hbar/2$?
	\\
	\\
	7. What is the probability that a measurement of spin along the u-axis will find $+\hbar/2$?
	\\
	\\
	8. Calculate the mean value $\braket{\psi|S_z|\psi}$ by two methods. First by using statistical
	analysis where the incident beam contains N silver atoms, and the direct
	calculation of the matrix element. How does this result compare with classical
	prediction?
	\\
	\\
	\textbf{Solution}
	...
	\]
	Since the state is prepared to be in the down state along $\hat u$, so the state before measurement would be:
	\[
		\ket{\psi} = \ket{-}_u=-\sin\frac{\theta}{2}\ket{+}+\cos\frac{\theta}{2}\ket{-}
	\]
	...
	\\
	\\
	4. Since the spin is along $\hat u$ which is only in the $x$-$z$ plane, there is zero probability that the spin will be found along the y-axis.
	\\
	\\
	5. 
	\[
		\begin{split}
		\ket{+}_x=&\frac{1}{\sqrt2}\left[ \ket{+}+\ket{-} \right] \\
		\mathcal P_{x+}=&\frac{1}{2}\abs{ \braket{+|\psi} + \braket{-|\psi} }^2 \\
		=& \frac{1}{2} \left( -\sin\frac{\theta}{2} + \cos\frac{\theta}{2} \right)^2 \\
		=& \frac{1}{2} \left( \sin^2\frac{\theta}{2} + \cos^2\frac{\theta}{2}-2\sin\frac{\theta}{2}\cos\frac{\theta}{2} \right) \\
		=&  \frac{1}{2}-\sin\frac{\theta}{2}\cos\frac{\theta}{2} \\
		=& \frac{1}{2}\left( 1-\sin\theta \right)
		\end{split}
	\]
	6. The probability is 100\% since we prepared the system in the down u-direction to begin with.
	\\
	\\
	7. Since the system is set up in the state down along $\hat u$, the probability to find it up along $\hat u$ is zero.
	\\
	\\
	8. First, using statistics, we need to know what possible outcomes there are from applying $S_z$ to $\ket{\psi}$ and with what probabilities. In parts 2 and 3 we explored this, so we already have the answers. Our two possible outcomes for measurements of $S_z$ are:
	\[
		s_{z+}=+\frac{\hbar}{2},\; s_{z-}=-\frac{\hbar}{2}
	\]
	With probabilities:
	\[
		\mathcal P_+=\sin^2\frac{\theta}{2},\; \mathcal P_-=\cos^2\frac{\theta}{2}
	\]
	And the average value for this series of measurements is given by:
	\[
		\begin{split}
		\braket{S_z} =& \sum s_{z\pm}\mathcal P_\pm\\
		=& \frac{\hbar}{2}\sin^2\frac{\theta}{2}+\left( -\frac{\hbar}{2} \right)\cos^2\frac{\theta}{2}\\
		=& \frac{\hbar}{2}\sin^2\frac{\theta}{2}-\frac{\hbar}{2}\cos^2\frac{\theta}{2} \\
		=& -\frac{\hbar}{2}\cos\theta
		\end{split}
	\]
	Now we want to do the same calculation but by using direct matrix element manipulation:
	\[
		\begin{split}
		\braket{S_z}=&\braket{\psi|S_z|\psi} = \frac{\hbar}{2}\begin{pmatrix} -\sin\frac{\theta}{2} & \cos\frac{\theta}{2}\end{pmatrix}
		\begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix} \begin{pmatrix} -\sin\frac{\theta}{2} \\ \cos\frac{\theta}{2}\end{pmatrix} \\
		=& \frac{\hbar}{2}\begin{pmatrix} -\sin\frac{\theta}{2} & \cos\frac{\theta}{2}\end{pmatrix} 
		\begin{pmatrix} -\sin\frac{\theta}{2} \\ -\cos\frac{\theta}{2}\end{pmatrix} \\
		=& \frac{\hbar}{2}\sin^2\frac{\theta}{2}-\frac{\hbar}{2}\cos^2\frac{\theta}{2} \\
		=& -\frac{\hbar}{2}\cos\theta
		\end{split}
	\]
	In the classical picture, you would not find two discrete values with various probabilities, but rather a continuous range of values between $\frac{\hbar}{2},-\frac{\hbar}{2}$. But the average of $S_z$ measurements in the quantum picture does not have to be one of the allowed states, and the probabilities that govern the two possible states are the sames for both the classical and quantum pictures. So even whether your system can take continuous or discrete values, the averages are calculated the same way. This also fits nicely with Ehrenfest's Theorem, which shows that the average values of quantum states behave like classical values.
	\\
	\\
	Therefore, in the classical picture:
	\[
		\bar S_z = -\frac{\hbar}{2}\cos\theta
	\]

\end{homeworkProblem} 
\pagebreak
\begin{homeworkProblem} 
	Consider a spin 1/2 particle placed in a magnetic field $\vec B=\begin{pmatrix} B_x & 0 & B_z \end{pmatrix}=\frac{B_0}{\sqrt2}\begin{pmatrix} 1 & 0 & 1 \end{pmatrix}$ in the orthonormal basis $\{\ket{+},\ket{-}\}$ for $S_z$.
	\\
	\\
	1. Calculate the matrix representing the Hamiltonian $H$ of the system.
	\\
	\\
	2. Calculate the eigenvalues and eigenvectors of $H$.
	\\
	\\
	3. The system at time $t=0$ is in the state $\ket{-}$. What values can be found if the energy is measured, and with what probabilities?
	\\
	\\
	4. Calculate the state vector $\ket{\psi(t)}$ at time $t$. At this instant, $S_x$ is measured; what is the mean value of the results that can be obtained? Give a geometrical interpretation of your results.
	\\
	\\
	\textbf{Solution}
	\\
	\\
	1. The Hamiltonian of a particle of spin 1/2 in a magnetic field is:
	\[
		\begin{split} 
		H=&-\vec{\mu}\cdot\vec B \\
		\vec{\mu}=&\gamma \vec S \\
		\Rightarrow H=&-\gamma\vec S\cdot\vec B \\
		=& -\gamma(S_xB_x+S_yB_y+S_zB_z) \\
		=& -\frac{\gamma B_0\hbar}{2\sqrt2} \left[\begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}+\begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix} \right] \\
		=& -\frac{\gamma B_0\hbar}{2\sqrt2} \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}
		\end{split} 
	\]
	2. By the usual means, calculate eigenvalues then eigenvectors:
	\[
		\begin{split}
		\begin{vmatrix} 1-\lambda & 1 \\ 1 & -(1+\lambda) \end{vmatrix} =& -(1+\lambda)(1-\lambda)-1=0\\
		=& -(1-\lambda^2)-1 = \lambda^2-2=0 \\
		\Rightarrow \lambda=\pm\sqrt2
		\end{split}
	\]
	So the eigenvalues of $H$ are:
	\[
		E_\pm = \mp\frac{\gamma B_0\hbar}{2} = \pm\frac{\omega_0\hbar}{2}
	\]
	where $\omega_0$ is the Larmor frequency.\\
	\\
	Now plug in these values to calculate the eigenvectors.
	\[
		\begin{split} 
		&\lambda=+\sqrt2 \\
		&\begin{pmatrix} 1-\sqrt2 & 1 \\ 1 & -(1+\sqrt2) \end{pmatrix}
		...
		\\
		\\
		&v_+= \begin{pmatrix} 1 \\ \sqrt2-1 \end{pmatrix} \\
		&v_-= \begin{pmatrix} 1 \\ -\sqrt2-1 \end{pmatrix}
		\end{split}
	\]
	3. At $t=0$ the system is in the $\ket{-}$ state, meaning $v_-$ from the last step.
	\[
		\ket{\psi(0)}=\begin{pmatrix} 0 \\ 1 \end{pmatrix} = \ket{-}
	\]
	To find the possible energies, we simply apply the Hamiltonian operator that was calculated in part 1.
	\[
		\begin{split} 
		H\ket{\psi(0)}=&-\frac{\gamma B_0\hbar}{2\sqrt2} \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}\begin{pmatrix} 0 \\ 1 \end{pmatrix} \\
		=&-\frac{\gamma B_0\hbar}{2\sqrt2}\begin{pmatrix} 1 \\ -1 \end{pmatrix} \\
		=& -\frac{\gamma B_0\hbar}{2\sqrt2} \ket{+}+\frac{\gamma B_0\hbar}{2\sqrt2}\ket{-} \\
		E_\pm =& \mp\frac{\gamma B_0\hbar}{2\sqrt2} \\
		\end{split} 
	\]
	But since the system is in the $\ket{-}$ state, there is zero possibility of finding it to have $E=E_+$ and a 100\% probability to find it to have energy $E=E_-$ state.
	\\
	\\
	4. 
	\[
		\begin{split}
		\ket{\psi(t)}=& -\frac{\gamma B_0\hbar}{2\sqrt2}e^{-i\omega_0t} \ket{+}+\frac{\gamma B_0\hbar}{2\sqrt2}e^{i\omega_0t}\ket{-} \\
		=& -\frac{\gamma B_0\hbar}{2\sqrt2} \begin{pmatrix} e^{-i\omega_0t} \\ -e^{i\omega_0t} \\
		 \end{pmatrix} \\
		\braket{S_x}=& \braket{\psi(t)|S_x|\psi(t)} \\
		=& \left( \frac{\gamma B_0\hbar}{2\sqrt2} \right)^2 \begin{pmatrix} e^{i\omega_0t} & -e^{-i\omega_0t}\end{pmatrix} \frac{\hbar}{2}\begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}\begin{pmatrix} e^{-i\omega_0t} \\ -e^{i\omega_0t} \\
		\end{pmatrix} \\
		=& \frac{\gamma^2B_0^2\hbar^3}{16} \begin{pmatrix} e^{i\omega_0t} & -e^{-i\omega_0t} \end{pmatrix} \begin{pmatrix} -e^{i\omega_0t} \\ e^{-i\omega_0t} \end{pmatrix} \\
		=& -\frac{\gamma^2B_0^2\hbar^3}{16} \left( e^{2i\omega_0t}+e^{-2i\omega_0t} \right) \\
		=& -\frac{\gamma^2B_0^2\hbar^3}{8}\cos2\omega_0t
		\end{split}
	\]
	The interpretation of this result is that the average x-component of the spin oscillates proportional to $\cos2\omega_0t$
