...
\begin{homeworkProblem}
    ...
    \[
	    \begin{split}
	    \abs{\mathcal{M}(D^0\rightarrow K^- \pi^+)}\propto cos^2\theta_c \approx 0.949 \\
	    \abs{\mathcal{M}(D^0\rightarrow \pi^- \pi^+)}\propto sin\theta_c cos\theta_c \approx 0.219 \\
	    \abs{\mathcal{M}(D^0\rightarrow \pi^- K^+)}\propto sin^2\theta_c \approx 0.0506
	    \end{split}
    \]
    Since the final states all have comparable masses, the biggest contributor to their different rates is the Cabibbo angle. The relative rates between them are:
    \[
	    \begin{split}
	    \abs{\mathcal{M}(D^0\rightarrow K^- \pi^+)}^2:\abs{\mathcal{M}(D^0\rightarrow \pi^- \pi^+)}^2:\abs{\mathcal{M}(D^0\rightarrow \pi^- K^+)}^2 \\
	    = 0.901:0.0480:0.00256 \\
	    = 1:0.0533:0.00284
	    \end{split}
    \]
    In the PDG, we see:
    \[
	    \Gamma(D^0\rightarrow K^- \pi^+):\Gamma(D^0\rightarrow \pi^- \pi^+):\Gamma(D^0\rightarrow \pi^- K^+)=1:0.0362:0.00356
    \] 
...
\begin{homeworkProblem}
	Given that the partial rate $\Gamma(K^+\rightarrow \pi^0e^+\nu) = 4 \times 10^6 s^{-1}$, calculate the rate for $D^0 \rightarrow K^-e^+\nu$.
	\\
	\\
	\textbf{Solution}
    Looking at their diagrams, they are very similar. The only difference is in the vertex with the cabibbo angle. Using the strengths of this vertices, we can estimate their relative rates:
    \[
	    \begin{split}
		\Gamma(K^+\rightarrow \pi^0e^+\nu) \propto \Delta m sin^2\theta_c \\
		\Gamma(D^0 \rightarrow K^-e^+\nu) \propto \Delta m cos^2\theta_c \\
		\Rightarrow \frac{\Gamma(K^+\rightarrow \pi^0e^+\nu)}{\Gamma(D^0 \rightarrow K^-e^+\nu)} = 6.42 \times 10^{-5} \\
		\Gamma(D^0 \rightarrow K^-e^+\nu) = \frac{\Gamma(K^+\rightarrow \pi^0e^+\nu)}{6.42 \times 10^{-5}}=6.234 \times 10^{10}\\
		\Rightarrow \tau = 1.6 \times 10^{-11} s
		\end{split}
    \]
    But this value differs by two orders of magnitude from the value given in the PDG.
\end{homeworkProblem}
